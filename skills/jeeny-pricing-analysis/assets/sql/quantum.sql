-- HOW BIG: shock rides, rate, total excess, avg and P90, per market and per day.
-- Money is per-market currency (SAR / JOD) — never combine.
-- Sanity: ~41% SA, ~26% JO. Baselines in references/baselines.md §2.
-- Set the window in `params` and say which window a figure came from.
...
SELECT country,
       ...
       ROUND(100.0 * SUM(IFF(fare_diff > 0.01, 1, 0)) / NULLIF(COUNT(*), 0), 2) AS pct_shock,
       ROUND(SUM(IFF(fare_diff > 0.01, fare_diff, 0)), 2)                AS total_excess,
       ROUND(AVG(IFF(fare_diff > 0.01, fare_diff, NULL)), 2)             AS avg_overcharge,
       ROUND(PERCENTILE_CONT(0.90) WITHIN GROUP (
                 ORDER BY IFF(fare_diff > 0.01, fare_diff, NULL)), 2)    AS p90_overcharge,
       SUM(IFF(fare_diff > 0 AND fare_diff <= 0.01, 1, 0))               AS rounding_gaps
FROM Classified
GROUP BY 1
ORDER BY 1;

-- Add createddate to GROUP BY (and to the SELECT) for the daily trend.
-- Add area_code for the city cut — always name the area in a finding.