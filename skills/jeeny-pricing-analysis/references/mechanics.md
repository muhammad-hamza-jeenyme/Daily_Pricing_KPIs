...
**`DETAILS`** — `CREATEDDATE` (Saudi date) · `AREA_CODE` · `BOARDED` · `REQUEST_SERVICE` · `MODEOFPAYMENT` · `CARDFLAG` · `OUTSTANDINGBALANCE` (digital underpay spillover; see `docs/payment-spillover-price-shocks.md`) · `SURGEMULTIPLIER`, `DISCRIMINATIONMULTIPLIER` (bucket 1) · `ACTUALDISCRIMINATIONMULTIPLIER` (unvalidated) · `SURCHARGE`, `VATONSURCHARGE`, `INTERCITYSURCHARGE`, `VATONINTERCITYSURCHARGE` (bucket 2) · `RIDEHAILINGSURCHARGE`(+VAT) · `TAXIMETERPRICE`, `TAXIMETERBOARDTOFINISHDISTANCE/DURATION` · `ESTIMATEDDISTANCE`, `ESTIMATEDTRAVELTIME`, `ESTIMATEDAMOUNT` · `REQUESTLAT/LONG`, `ACTUALPICKUPLAT/LNG`, `ACTUALDROPOFFLAT/LON`, `REQUESTEDDROPOFFLAT/LONG` · `ADDRESSSOURCESPICKUP`/`DESTINATION` · waiting mechanics `TOTALWAITINGTIME`, `CHARGEDWAITINGTIME`, `COURTESYTIME`, `MAXCHARGEABLEMINUTES`, `WAITTIMEFACTOR`, `WAITINGTIMEFEE` · `DESTINATIONAREACODE`, `REQUESTEDDESTINATIONAREACODE` (intercity) · `TRACEID`. Receipt-mirroring columns exist here — prefer `RECEIPTS`.
